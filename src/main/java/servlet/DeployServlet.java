		...
		if (request.getParameter("option") != null) {
			...
			case 0:
				...
				if (request.getParameter("selectImage") != null
						&& !request.getParameter("selectImage").equalsIgnoreCase("-1")) {
				...
				forward(DEPLOY_SINGLE_OS, request, response);
				return;
			case 1:
			...
			case 2:
				...
				ArrayList<ImageDTO> imageActiveList = ImageDAO.getImageActiveList();
				request.setAttribute("activeImageList", imageActiveList);
				...
				if (deployImageIdList != null) {
					...
					for (ClientDTO x : macDeployClientList) {	
						System.out.println("\nMAC - Client: " + x.getMac() + "\nDeploy Image  - Client: " + x.getMacDeployImage() + 
								"\nDeploy Image - JSP send to Servlet: " + deployImageIdList[in]);
						in += 1;
					}
					if (deployImageIdList.length != macDeployClientList.size()) {
						System.out.println(
								"Error: deployImageIdList.length != macDeployClientList.size() in DeployServlet"
										+ "!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!");
					} else {
				...
				forward(DEPLOY_OS_WITHIN_CLIENTMAC, request, response);
				return;
			}
